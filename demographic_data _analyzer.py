...
def calculate_demographic_data(print_data=True):
    ...
    higher_education = df.loc[((df['education'] == 'Bachelors') | (df['education'] == 'Masters') | 
         (df['education'] == 'Doctorate')), 'education'].loc[df['salary'] == '>50K'].count() / df.loc[((df['education'] == 'Bachelors') | (df['education'] == 'Masters') | 
         (df['education'] == 'Doctorate')), 'education'].count()
    
    higher_education_rich = round(higher_education*100, 1)
    ...
    lower_education_rich = round(lower_education*100, 1)
    
    # What is the minimum number of hours a person works per week (hours-per-week feature)?
    min_work_hours = df['hours-per-week'].min()

    # What percentage of the people who work the minimum number of hours per week have a salary of >50K?
    num_min_workers = df.loc[df['hours-per-week'] == 1, 'hours-per-week'].loc[df['salary'] == '>50K'].count()/df.loc[df['hours-per-week'] == 1, 'hours-per-week'].count()
    ...
    highest_earning_country = highest_earning_country.index[highest_earning_country.argmax()]
